...
public class ModDownloader {
    ...
    public ModDownloader(File destinationDirectory, boolean useFileCount) {
        int maxThreads = Math.max(1, AllSettings.getMaxDownloadThreads().getValue());
    ...
    class FileInfoQueryTask implements Runnable {
        ...
        public void run() {
            try {
                ...
                new DownloadTask(
                        new String[]{fileInfo.url},
                        new File(mDestinationDirectory, fileInfo.relativePath),
                        fileInfo.sha1
                ).run();
            } catch (IOException e) {
                downloadFailed(e);
    ...
    class DownloadTask implements Runnable, Tools.DownloaderFeedback {
        ...
        public DownloadTask(String[] downloadurls, File downloadDestination, String downloadHash) {
        ...
        public void run() {
            for (String sourceUrl : mDownloadUrls) {
                try {
                    ...
                    // Success: stop trying other URLs for the same file.
                    return;
                } catch (IOException e) {
                    downloadFailed(e);
                    return;
        ...
        private IOException tryDownload(String sourceUrl) throws InterruptedIOException {
            ...
            for (int i = 0; i < 5; i++) {
                try {
                    ...
                    if (mUseFileCount) mDownloadProgress.incrementAndGet();
                    return null;
                    ...
                    Logging.e(
                            "ModDownloader",
                            "Download failed for " + sourceUrl
                                    + " -> " + mDestination.getAbsolutePath()
                                    + " (parent=" + (mDestination.getParentFile() != null ? mDestination.getParentFile().getAbsolutePath() : "null") + ")\n"
    ...
    /**
     * Listener for downloaded file count and total downloaded file size.
     */
