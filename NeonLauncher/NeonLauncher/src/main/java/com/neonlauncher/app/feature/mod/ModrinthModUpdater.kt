package com.neonlauncher.app.feature.mod
...
object ModrinthModUpdater {
    ...
    fun updateSingleMod(
        ...
        val updateInfo = ModrinthUpdateChecker.checkForUpdate(
            context = context,
            file = installedFile,
        ...
        when (updateInfo.status) {
            ...
            ModrinthUpdateChecker.UpdateStatus.UNKNOWN -> {
                return UpdateActionResult(
                    status = UpdateActionStatus.UNKNOWN,
                    oldFile = installedFile,
                    newFile = null,
                    message = "Could not determine update information for this mod."
                )
            }

            ModrinthUpdateChecker.UpdateStatus.UPDATE_AVAILABLE -> Unit
        }
        ...
        return try {
            ...
            Logging.e("ModrinthModUpdater", "Failed to update ${installedFile.absolutePath}", t)
