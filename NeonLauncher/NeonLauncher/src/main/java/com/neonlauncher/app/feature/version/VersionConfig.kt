...
import com.neonlauncher.app.feature.version.VersionsManager.getNeonVersionPath
import com.neonlauncher.app.setting.AllSettings
import com.neonlauncher.app.utils.stringutils.StringUtils.getStringNotNull
...
class VersionConfig(private var versionPath: File) : Parcelable {
    ...
    fun saveWithThrowable() {
        ...
        val neonVersionPath = getNeonVersionPath(versionPath)
        if (!neonVersionPath.exists() && !neonVersionPath.mkdirs()) {
            throw IllegalStateException("Failed to create version config directory: ${neonVersionPath.absolutePath}")
        }

        val configFile = File(neonVersionPath, "VersionConfig.json")
    ...
    companion object CREATOR : Parcelable.Creator<VersionConfig> {
        ...
        fun parseConfig(versionPath: File): VersionConfig {
            val neonVersionPath = getNeonVersionPath(versionPath)
            val oldConfigFile = File(neonVersionPath, "NeonVersion.cfg")
            val configFile = File(neonVersionPath, "VersionConfig.json")
            ...
            // Missing config is normal for fresh/manual/non-Neon versions
